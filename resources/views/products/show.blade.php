...
@section('content')
    <section class="bg-white py-12">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <nav class="font-mono text-xs uppercase tracking-widest text-graphite-500">
                <a href="{{ route('home') }}" class="hover:text-steel-700">Beranda</a>
                <span class="mx-2">/</span>
                <a href="{{ route('products.index') }}" class="hover:text-steel-700">Produk</a>
                <span class="mx-2">/</span>
                <span class="text-graphite-900">{{ $product->name }}</span>
            </nav>
            ...
            <div class="mt-8 grid gap-10 lg:grid-cols-2">
                ...
                <div>
                    ...
                    <h1 class="mt-2 font-display text-3xl font-bold text-graphite-900 sm:text-4xl">{{ $product->name }}</h1>
                    ...
                    <div class="mt-8 flex flex-col gap-4 sm:flex-row">
                        ...
                        <a href="{{ route('products.index') }}" class="inline-flex items-center justify-center rounded border border-line-200 px-8 py-3.5 font-mono text-xs font-semibold uppercase tracking-widest text-graphite-500 transition hover:border-steel-700 hover:text-steel-700">
                            Lihat Produk Lain
                        </a>
                    </div>
                </div>
                        ...
                        <div class="overflow-x-auto">
                            <dl class="font-mono text-sm">
                                @foreach ($product->specifications as $index => $spec)
                                    <div class="flex flex-col gap-1 px-5 py-3.5 sm:flex-row sm:gap-0 {{ $index % 2 ? '' : 'bg-paper-100' }}">
                                        <dt class="w-full font-semibold text-graphite-900 sm:w-1/2">{{ $spec->spec_key }}</dt>
                                    ...
                                    @if (str_contains($video->video_url, 'youtube.com') || str_contains($video->video_url, 'youtu.be'))
                                        @php
                                            $videoId = str_contains($video->video_url, 'youtu.be')
                                                ...
                                                : substr(parse_url($video->video_url, PHP_URL_QUERY) ?: '', strpos(parse_url($video->video_url, PHP_URL_QUERY), 'v=') + 2, 11);
                                        @endphp
                                        <iframe class="h-full w-full" src="https://www.youtube.com/embed/{{ $videoId }}" title="{{ $video->caption }}" frameborder="0" allowfullscreen></iframe>
                                    @else
                                        <video class="h-full w-full" controls src="{{ $video->video_url }}"></video>
                                    @endif
                                </div>
                                @if ($video->caption)
                                    <p class="px-4 py-3 font-mono text-xs uppercase tracking-widest text-graphite-500">{{ $video->caption }}</p>
                                @endif
                            </div>
                        @endforeach
                    </div>
    ...
    <script>
        ...
        thumbs.forEach((thumb) => {
            thumb.addEventListener('click', () => {
                thumbs.forEach((t) => t.classList.remove('border-steel-700', 'ring-2', 'ring-steel-700/30'));
