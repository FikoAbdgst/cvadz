...
Sales website (Laravel 13) for CV Adzra Engineering Bandung (industrial machine fabrication). **MVP-complete** (public catalog + admin panel + staff panel). `PROJECT_CONTEXT.md` = schema & MVP spec; **`DESAIN.md` = visual authority** (supersedes PROJECT_CONTEXT §3 palette). Read both before structural changes.
...
- **Operational entities:** `services`, `suppliers`, `workers`, `attendances`, `cashbooks` (+ `warranty_months` on products, `admin_user_id`/`total`/`warranty_end_date` on orders, `staff_user_id`/`payment_type` on transactions, `transaction_id` FK on cashbooks). Public route `GET /layanan` = `ServiceController@index`, view `resources/views/services/index.blade.php`. `services` has no CRUD yet (planned); workers/attendances/cashbook input live in the staff panel. Demo staff login: `staff@cvadz.com` / `staff123` (role `staff`). Spareparts are a product category (`sparepart`), not a separate table.
- **Part 2 (admin Dashboard — Master Data & HR/Keuangan):**
  ...
  - Admin sidebar groups: **Master Data** (Kategori, Produk, Supplier) · **Penjualan** (Transaksi, Laporan, Cek Garansi) · **HR & Keuangan** (Buku Kas, Kelola Akun = `admin.users.*`, Rekap Absensi = `admin.attendances.index` read-only, Penggajian = `admin.payrolls.*`).
  - **Payroll flow:** POST `admin.payrolls.generate` (period Y-m) computes `total_days` from `attendances` (check_in present) × salary → draft rows; POST `admin.payrolls.approve` flips status to approved (approved_by/at) **and inserts a `cashbooks` pengeluaran row** (user_id = approver); DELETE only works on drafts.
...
- **Part 4 (Staff Operasional panel, role `staff`):**
  ...
  - There is **no "Proses Pesanan" sidebar menu** — it's merged into "Daftar Transaksi" (`staff.transactions.index`), which has tabs `?tab=pesanan|transaksi` (default `pesanan`): the `pesanan` tab lists orders that are **paid but not yet transacted by staff** (with proof photo preview, ACC button, Progress/Faktur links); the `transaksi` tab is the transaction history + filters.
...
- Palette (not Tailwind defaults): steel-900 `#0F2A42` (dark navbar/hero), steel-700 `#1C4C78` (primary actions), steel-400 `#5C86AC` (secondary), paper-100 `#F2F5F7` (page bg), graphite-900 `#1B222B` (ink), graphite-500 `#5C6773` (muted), line-200 `#D7E0E6` (borders), amber-600 `#D98A2B` (accent).
...
- Grep hits for **`admin.transactions.*` in `storage/framework/views/*.php` are stale compiled cache** from an older build — those routes don't exist anymore and can mislead you. Trust `routes/web.php` + `resources/views/` over compiled views.
